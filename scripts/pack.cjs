...
run(process.execPath,[require.resolve('electron-builder/cli.js'),'--publish','never',...(target.startsWith('mac-')?['--mac','dir']:['--win','zip']),target.endsWith('arm64')?'--arm64':'--x64',...(target===`mac-${process.arch}` && process.platform==='darwin' ? ['--config.electronDist='+path.join(path.dirname(require.resolve('electron')),'dist')] : [])]);
