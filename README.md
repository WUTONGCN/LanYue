...
- Apple 芯片 Mac：解压 `LanYue-0.1.6-mac-arm64.zip`，打开 `LanYue.app`。不必移动到“应用程序”，也不需要另装 Java 或 LibreOffice。
- Windows x64：将 `LanYue-0.1.6-win-x64.zip` **完整解压**，打开 `LanYue.exe`。不要单独复制 EXE，也不要在压缩软件内直接运行。尚未进行 Windows 实机验收。
...
例如当前 `0.1.6` 会变成 `0.1.7`，同时更新 `package.json`、锁文件，创建版本提交和 `v0.1.7` 标签。也可使用 `npm version minor` 或 `npm version 1.0.0`。标签必须与源码版本一致；推送标签后，两端构建全部成功才会生成包含 ZIP、SHA256 校验和与更新说明的 **Release 草稿**。在 Releases 检查草稿并点击 Publish release 后，用户即可下载，已安装的览阅也会检测到新版。已发布版本的文件不会被重跑流程覆盖，修正应使用新版本号。
...
自动构建完成不代表文件预览、系统集成或全部格式验收通过。此流程不运行桌面交互测试，也不提供发行签名、公证。开发预览版仍保留前述 CAD 评估限制和第三方组件声明。

## 应用内更新

从 `0.1.6` 起，览阅启动后会在正常窗口显示时检查 GitHub 最新正式 Release，并每 6 小时检查一次。后台空格预览不弹更新提示，网络不可用时自动检查保持安静。右上角「··· → 检查更新」可随时手动检查，Mac 应用菜单也有同一入口。没有新增首页按钮或设置页。

发现新版后可选择下载或稍后；下载期间仍可预览文件，菜单和 Dock／任务栏显示进度，点击下载菜单项可取消。完整 ZIP 下载完成后校验 SHA256、包内版本、平台架构和文件路径，再提示「重启并更新」。选择稍后会保留已准备好的包，下次打开无需重新下载。应用关闭后替换程序文件并重新启动；若文件替换或新窗口启动失败，更新组件会尝试恢复旧版。

更新保留原文件、默认应用数据和 Windows 的 `portable-data`。需要当前程序所在目录可写，并有下载、解压所需的临时磁盘空间；Mac 若仍运行在系统临时隔离目录，应先将应用移到固定文件夹后重新打开。此机制沿用免安装便携包，使用 GitHub HTTPS 与发布资产的 SHA256 校验，并非发行签名、公证或增量更新。失败记录保存在应用数据目录的 `updates/`，恢复失败时保留应用旁（Windows 为 EXE 同级）的 `.lanyue-update-*` 备份目录。

`0.1.5` 及更早版本没有更新入口，需要手动下载并替换一次 `0.1.6` 或更新版本；之后可以在应用内升级。Release 草稿、预发布版本和仅在 Actions 中的构建产物不会推送给现有用户。升级操作的 Mac／Windows 实机验收尚未完成。
